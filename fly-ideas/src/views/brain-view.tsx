...
export function BrainView({ ideas, onAddJournalEntry }: Props) {
 ...
 <TabsTrigger value="env">Окружение</TabsTrigger>
...
function FolderBar({ kind, label, paths, onChanged }: { kind: DirKind; label: string; paths: Paths | null; onChanged: () => void }) {
 ...
 return (
 <div className="flex flex-wrap items-center gap-1.5 border px-2 py-1 helper-label">
 ...
 <Button size="sm" variant="ghost" className="h-7" onClick={() => cur && fly!.openPath(cur)} title="Открыть в проводнике">Открыть</Button>
 {!isDefault && <Button size="sm" variant="ghost" className="h-7" title="Вернуть папку по умолчанию" onClick={async () => { await fly!.resetFolder(kind); onChanged(); }}>Сбросить</Button>}
 </div>
 );
}
...
function WebFallback() {
 ...
 Запусти <code className="bg-muted px-1">START.bat</code>: откроется окно, и здесь появятся кнопки «Скачать», «Запустить» и чат с помощником.
 </CardContent>
 </Card>
 </div>
 );
...
function DataPanel() {
 ...
 if (r.size < 2000 && item.sizeMB > 1) setErrors((e) => ({ ...e, [item.id]: "Скачался крошечный файл: скорее всего это заглушка Git LFS. Скачай файл вручную из репозитория (кнопка «Источник»)." }));
 } catch (e) {
 ...
 {missing ? `Скачать мозг мухи (${missing} файл.)` : "Всё скачано"}
 </Button>
 <Button variant="outline" onClick={dlAllTo} disabled={Object.values(busy).some(Boolean)} title="Скачать данные в другую папку (например, на большой диск)">Скачать в папку…</Button>
 ...
 Файлы берутся из открытого репозитория <a className="underline" href="https://github.com/eonsystemspbc/fly-brain" target="_blank" rel="noreferrer">eonsystemspbc/fly-brain</a>: FlyWire v783 (последний публичный релиз коннектома, тот же, что в Nature 2024) и код модели Shiu et al. Полный мозг занимает около 100 МБ таблиц, не терабайты: сырые ЭМ-снимки для симуляции не нужны. Ссылки проверены 18.09.2026.
 </p>
 {groups.map((g) => (
 ...
 return (
 <div key={it.id} className="rounded-[2px] border px-3 py-2">
 ...
 <Button size="icon-sm" variant="ghost" title="Открыть страницу файла на GitHub" onClick={() => fly!.openExternal(it.url.replace("raw.githubusercontent.com", "github.com").replace("/main/", "/blob/main/"))}>Источник</Button>
 {busy[it.id] ? (
 <Button size="sm" variant="outline" onClick={() => fly!.cancelDownload(it.id)}>{pct ?? "…"}%</Button>
 ) : (
 <>
 <Button size="sm" variant="ghost" title="Скачать в другую папку" onClick={() => dlTo(it)}>В папку</Button>
 <Button size="sm" variant={it.present ? "ghost" : "default"} onClick={() => dl(it)}>{it.present ? "Обновить" : "Скачать"}</Button>
 </>
 )}
 </div>
 {pr && <div className="mt-2 h-1 overflow-hidden rounded-none bg-muted"><div className="h-full bg-[var(--chart-1)] " style={{ width: `${pct ?? 5}%` }} /></div>}
 ...
 <CardDescription className="text-xs">Официальные выгрузки и аннотации: обычно требуют аккаунта или лежат не одним файлом. Открываются в браузере; скачанное можно положить в папку данных.</CardDescription>
 </CardHeader>
 <CardContent className="flex flex-col gap-1.5">
 {external.map((e) => (
 <button key={e.url} onClick={() => fly!.openExternal(e.url)} className="flex items-center gap-2 rounded-[2px] border px-3 py-2 text-left text-sm hover:bg-accent cursor-pointer">
...
function EnvPanel() {
 ...
 <Button variant="outline" onClick={check} disabled={loading}>Проверить снова</Button>
 </div>
 {env && (
 ...
 <p className="text-muted-foreground">Проще всего поставить <a className="underline" href="https://docs.anaconda.com/miniconda/" target="_blank" rel="noreferrer">Miniconda</a> (Windows 64-bit). При установке поставь галочку «Add to PATH». Затем перезапусти программу.</p>
 </div>
 )}
 <div>
 <div className="font-medium">{env?.python ? "1" : "2"}. Пакеты для CPU-модели (Anaconda Prompt или PowerShell)</div>
 <pre className="mt-1 overflow-x-auto rounded-[2px] bg-muted p-2 font-mono text-xs">pip install numpy pandas pyarrow scipy matplotlib brian2</pre>
 </div>
 <div>
 <div className="font-medium">{env?.python ? "2" : "3"}. GPU-бэкенды (потом, когда упрёшься в скорость)</div>
 <p className="text-muted-foreground">PyTorch с CUDA: <code className="bg-muted px-1">pip install torch --index-url https://download.pytorch.org/whl/cu124</code>. Brian2CUDA и GeNN ставятся через WSL2, скрипт лежит в репозитории Eon (<code className="bg-muted px-1">scripts/setup_WSL_CUDA.sh</code>).</p>
 </div>
 </CardContent>
 </Card>
 </div>
...
function Row({ ok, label, value, soft }: { ok: boolean; label: string; value: string; soft?: boolean }) {
 return (
 <div className="flex items-center gap-2">
 <span className={cn("size-2 rounded-none", ok ? "bg-[var(--chart-2)]" : soft ? "bg-muted-foreground/40" : "bg-destructive")} />
...
function ScriptsPanel() {
 ...
 <Button size="sm" variant="ghost" onClick={importScript} title="Добавить .py с диска">Добавить .py</Button>
 </CardHeader>
 <CardContent className="flex flex-col gap-1">
 {scripts.map((s) => (
 <button key={s.name} onClick={() => open(s.name)} className={cn("rounded-[2px] px-2 py-1.5 text-left text-sm hover:bg-accent cursor-pointer", sel === s.name && "bg-accent")}>
 <div className="font-mono text-xs">{s.name}</div>
 {s.doc && <div className="line-clamp-2 text-[11px] text-muted-foreground">{s.doc}</div>}
 </button>
 ))}
 <p className="mt-2 text-[11px] text-muted-foreground">Положи свой .py в папку, и он появится в списке. Скрипт получает переменные окружения FLY_DATA, FLY_RUN_DIR; если он запишет summary.json в FLY_RUN_DIR, результат подхватится во вкладке «Прогоны».</p>
 </CardContent>
 </Card>
 ...
 <Button variant="ghost" size="sm" onClick={exportScript} title="Сохранить копию в другое место"> Копия…</Button>
 <Button variant="outline" size="sm" onClick={save} disabled={!dirty}>Сохранить</Button>
 {runId ? (
 <Button size="sm" variant="destructive" onClick={() => fly!.killRun(runId)}>Остановить</Button>
 ) : (
 <Button size="sm" onClick={run}> Запустить</Button>
 )}
 </div>
 </div>
 ...
 <pre ref={logRef} className="max-h-[300px] min-h-[120px] overflow-auto rounded-[2px] border bg-muted/40 p-3 font-mono text-[12px] whitespace-pre-wrap">{log || "Вывод появится здесь."}</pre>
 </>
 ) : (
 <div className="flex h-full min-h-[200px] items-center justify-center rounded-[2px] border border-dashed text-sm text-muted-foreground">Выбери скрипт слева. Начни с 00_check_env.py.</div>
 )}
 </div>
 </div>
 </div>
...
function RunsPanel({ ideas, onAddJournalEntry }: Props) {
 ...
 {runs.map((r) => (
 <button key={r.dir} onClick={() => setSel(r)} className={cn("rounded-[2px] px-2 py-1.5 text-left hover:bg-accent cursor-pointer", sel?.dir === r.dir && "bg-accent")}>
 <div className="font-mono text-[11px]">{r.dir}</div>
 <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">{(r.summary ? "summary.json" : "без summary")} · файлов: {r.files.length}{r.files.some((f) => /\.(png|jpe?g|svg)$/i.test(f)) ? " · есть графики" : ""}</div>
 </button>
 ))}
 </CardContent>
 </Card>
 ...
 <Button size="sm" variant="ghost" onClick={() => fly!.openPath(sel.path)}> Папка</Button>
 <Button size="sm" variant="ghost" title="Скопировать папку прогона в другое место" onClick={async () => { const p = await fly!.exportRun(sel.dir); if (p) alert(`Скопировано: ${p}`); }}> Экспорт…</Button>
 ...
 <Button size="sm" onClick={toJournal} disabled={!ideaId}>{added === sel.dir ? <> Добавлено</> : <> В журнал идеи</>}</Button>
 </div>
 </div>
 {images.length > 0 && (
 <div className="grid gap-3 md:grid-cols-2">
 {images.map((im) => (
 <figure key={im.name} className="overflow-hidden border bg-card">
 ...
 {otherFiles.map((f) => (
 <button key={f} onClick={async () => { const r = await fly!.readRunFile(sel.dir, f); if (r.kind === "text") setViewFile({ name: f, text: r.text }); }} className={cn("rounded-[2px] border px-2 py-1 font-mono text-[11px] hover:bg-accent cursor-pointer", viewFile?.name === f && "bg-accent")}>{f}</button>
 ))}
 </div>
 )}
 {viewFile && <pre className="max-h-[300px] overflow-auto rounded-[2px] border bg-muted/40 p-3 font-mono text-[11px] whitespace-pre-wrap">{viewFile.text}</pre>}
 {detail?.summary && (
 <pre className="max-h-[300px] overflow-auto rounded-[2px] border bg-muted/40 p-3 font-mono text-[12px]">{JSON.stringify(detail.summary, null, 2)}</pre>
 )}
 <pre className="max-h-[360px] overflow-auto rounded-[2px] border p-3 font-mono text-[12px] whitespace-pre-wrap text-muted-foreground">{detail?.log || "лог пуст"}</pre>
 </>
 ) : (
 <div className="flex min-h-[200px] items-center justify-center rounded-[2px] border border-dashed text-sm text-muted-foreground">Выбери прогон, чтобы посмотреть результат и отправить его в журнал идеи.</div>
 )}
 </div>
 </div>
 </div>
...
Правила: отвечай по-русски, кратко и конкретно. Если пишешь код, то это Python: полный запускаемый файл, с argparse, читающий пути из переменных окружения FLY_DATA (данные) и FLY_RUN_DIR (куда писать summary.json и графики). Не выдумывай API Brian2: если не уверен, скажи, что нужно проверить в документации. Честно указывай ограничения LIF-модели.`;
...
function AssistantPanel({ ideas }: { ideas: Idea[] }) {
 ...
 <button key={q} onClick={() => setInput(q)} className="border px-2 py-1 font-mono text-[10px] uppercase tracking-[0.05em] text-muted-foreground hover:bg-accent hover:text-foreground cursor-pointer">{q}</button>
 ))}
 </div>
 </div>
 )}
 {msgs.map((m, i) => <Message key={i} m={m} onSaveCode={saveCodeBlock} />)}
 {busy && <div className="flex items-center gap-2 text-xs text-muted-foreground">запрос отправлен…</div>}
 {err && <div className="rounded-[2px] border border-destructive/40 bg-destructive/10 p-2 text-xs text-destructive">{err}</div>}
 <div ref={endRef} />
 </div>
 <div className="flex items-end gap-2 border-t p-3">
 <Textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder="Сообщение: Enter отправляет, Shift+Enter переносит строку" className="min-h-[44px] max-h-[160px]" disabled={!key} />
 <Button onClick={send} disabled={!key || busy || !input.trim()}>Отправить</Button>
 </div>
 </Card>
 <div className="flex flex-col gap-3">
 <Card className="gap-2">
 <CardHeader><CardTitle className="text-sm">DeepSeek API</CardTitle></CardHeader>
 <CardContent className="flex flex-col gap-2 text-xs">
 {key && !showKey ? (
 <div className="flex items-center gap-2"><span className="size-2 rounded-none bg-[var(--chart-2)]" /><span className="font-mono">{key}</span><Button size="sm" variant="ghost" className="ml-auto h-6 px-2" onClick={() => setShowKey(true)}>Сменить</Button></div>
 ) : (
 <>
 ...
 {key && <Button size="sm" variant="ghost" className="h-7" onClick={() => setShowKey(false)}>отмена</Button>}
 </div>
 </>
 )}
 ...
 <p className="text-[11px] text-muted-foreground">Если в ответе есть блок кода Python, под ним появится кнопка «Сохранить как скрипт»: файл попадёт в папку скриптов, и его можно сразу запустить. Чтобы задать имя, добавь в первую строку кода комментарий <code className="bg-muted px-1"># file: имя.py</code>.</p>
 </div>
 </div>
 );
}
...
function Message({ m, onSaveCode }: { m: LlmMessage; onSaveCode: (code: string) => void }) {
 ...
 <div className="label mb-1.5">{m.role === "user" ? "запрос" : "ответ помощника"}</div>
 <div className="max-w-[92%] text-[13px]">
 {parts.map((p, i) =>
 p.type === "text" ? (
 <div key={i} className="whitespace-pre-wrap leading-relaxed">{p.content.trim()}</div>
 ) : (
 <div key={i} className="my-2">
 <pre className="overflow-x-auto rounded-[2px] border bg-background p-2.5 font-mono text-[12px] text-foreground">{p.content}</pre>
 {(p.lang ?? "").match(/^py/i) && (
 <Button size="sm" variant="outline" className="mt-1" onClick={() => onSaveCode(p.content)}>Сохранить как скрипт</Button>
 )}
 </div>
 ),
 )}
